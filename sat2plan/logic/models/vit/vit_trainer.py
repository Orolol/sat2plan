...
from sat2plan.logic.models.vit.vit_model import ViTSat2Plan
...
class ViTTrainer:
    def __init__(self, rank, world_size):
        try:
            ...
            self.learning_rate = self.M_CFG.learning_rate_ViT  # Using generator learning rate
    ...
    def create_model(self):
        # Initialize ViT model with correct image size
        self.model = ViTSat2Plan(
            img_size=self.image_size,  # Using the image size from config
        ...
        print(f"Patch size: 16x16 ({(self.image_size // 16) ** 2} patches)\n")
        ...
        self.scheduler = torch.optim.lr_scheduler.OneCycleLR(
            ...
            pct_start=0.5,  # 10% warmup
            div_factor=5.0,  # initial_lr = max_lr/25
    ...
    def train(self):
        try:
            ...
            for epoch in range(self.starting_epoch, self.n_epochs):
                ...
                for idx, (x, y, _) in enumerate(self.train_dl):
                    ...
                    with torch.cuda.amp.autocast():
                        y_pred = self.model(x)
                        loss = self.L1_Loss(y_pred, y) + 0.1 * self.MSE_Loss(y_pred, y)
                    ...
                    if self.rank == 0 and idx % 10 == 0:
                        ...
                        if idx % 100 == 0:
                            with torch.no_grad():
                                with torch.cuda.amp.autocast():
                                    test_output = self.model(x[:4])
                                    img_sample = torch.cat((x[:4], test_output[:4], y[:4]), dim=2)
                                save_image(img_sample, f"images/{epoch}-{idx}.png", nrow=3, normalize=True)
