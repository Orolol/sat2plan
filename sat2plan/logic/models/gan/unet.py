    ...
    def train(self):
        ...
        for epoch in range(self.n_epochs):
            ...
            if self.save_model_bool and (epoch+1) % 5 == 0:
                if epoch < 11 or (self.val_Gen_loss[-1] + self.val_Dis_loss[-1] < sum([self.val_Gen_loss[:-1][i]+self.val_Gen_loss[:-1][i] for i in range(len(self.val_Gen_loss[:-1]))])/len(self.val_Gen_loss)):
