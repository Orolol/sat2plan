...
class Discriminator(nn.Module):
    def __init__(self, kernel_size, stride, padding, in_channels=3, features = [64,128,256,512]):
        ...
        self.initial = nn.Sequential(
            nn.Conv2d(in_channels*2, features[0], kernel_size, stride, padding, padding_mode="reflect"),
            nn.LeakyReLU(0.2)
        )
        ...
        for feature in features[1:]:
            layers.append(
                CNN_Block(in_channels, feature, stride=1 if feature==features[-1] else stride)
...
class Generator(nn.Module):
    def __init__(self,  kernel_size, stride, padding, in_channels=3,features=64):
        ...
        self.bottleneck = nn.Sequential(
            nn.Conv2d(features*8,features*8, kernel_size, stride, padding,padding_mode="reflect"),                      # 1 X 1
            nn.ReLU()
        )
        ##############################################################################
        ################################## DECODEUR ##################################
